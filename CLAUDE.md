...
- Ошибки/HTTP-коды не документированы — поведение фиксируется фактом при smoke-тестах
  (обновлять `info/00_api_methods.md`).
- Формат ответа: только CSV; кодировка/разделитель подтверждаются фактом на первом
  smoke-тесте.
...
- `account_id = 1`, `source_type_id` — примеры, заменяемые при интеграции
  (значение `source_type_id` для Bidease уточняется на этапе спеки)
...
- `costs_nds` ← API `spend` — **десятичный тип (float), округление до 2 знаков**
- `costs_without_nds = costs_nds / (1 + ставка_НДС)` — ставка по году даты строки:
  год ≥ 2026 → 22% (делитель 1.22), ранее → 20% (делитель 1.20)
- `costs_nds_ak = costs_nds * 1.5`; `costs_without_nds_ak = costs_without_nds * 1.5`

> ⚠️ **`spend` у Bidease — в долларах США** (медийный расход) и, в отличие от Avito,
> вопрос «с НДС или без» в доках не раскрыт. Применимость НДС-производных к долларовому
> спенду — **открытый вопрос, решается при спеке `get_campaigns_daily_stat`** (варианты:
> считать по конвенции механически / зафиксировать курс-пересчёт на стороне клиента).
> Решение зафиксировать здесь и в `info/01_functions_implemented.md`.

**Составные ключи:**
- `id_key_camp = "1_" + campaign_id` — для всех функций
- `id_key_ad` — для ad-level (креативы); в Bidease нет групп → формула без group-звена
  (предварительно `id_key_camp + "_" + creative_id`; финализируется в спеке)
