...
> (только базовые: показы/клики/расход). `source_type_id = 10`.
>
> ⚠️ **Ревизия 2026-07-27 (отменяет решение 2026-07-21 по расходам):** `spend` —
> **доллары США**, а `costs_*`-поля конвенции рублёвые → в итоговых таблицах их НЕТ.
> Расход отдаётся одним полем `costs_usd` ← `spend` как есть (round(2)); константа `ak`
> и расчёт НДС **не применяются**. Упоминания `costs_nds`/`costs_without_nds`/`*_ak`
> ниже по файлу — исторические (анкета заполнялась 21.07); актуальный контракт —
> `specs/02–04` и `info/01_functions_implemented.md`.
