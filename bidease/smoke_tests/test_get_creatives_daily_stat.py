"""Smoke-тест get_creatives_daily_stat — живой API Bidease, требует bidease/.env.

Запуск из корня репо: python bidease/smoke_tests/test_get_creatives_daily_stat.py
Даты — из .env (TEST_START_DATE / TEST_END_DATE, включительно). Результат —
bidease/raw_data/get_creatives_daily_stat_{from}_{to}.csv (cp1251); показ первых
...
from bidease import get_creatives_daily_stat  # noqa: E402
...
def main() -> None:
    ...
    df = get_creatives_daily_stat(date_from, date_to)
    print(f"get_creatives_daily_stat() -> shape={df.shape}")

    out_dir = PROJECT_ROOT / "raw_data"
    out_dir.mkdir(parents=True, exist_ok=True)
    for old in out_dir.glob("get_creatives_daily_stat_*.csv"):
        old.unlink()
    out_path = out_dir / f"get_creatives_daily_stat_{date_from}_{date_to}.csv"
